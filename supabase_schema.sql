-- SUPABASE SCHEMA SETUP FOR STUDENT WELLNESS TRACKER
...
create policy "Users can update own profile (except role)" on public.profiles
  ...
  with check (
    auth.uid() = auth_id
    and role = (select role from public.profiles where auth_id = auth.uid()) -- Prevents changing role
  );
...
create policy "Students can update own progress" on public.daily_progress
  ...
  with check (
    auth.uid() = student_id 
    and admin_status = (select admin_status from public.daily_progress where id = daily_progress.id) -- prevents students from changing admin_status
  );
...
create view public.daily_progress_analytics as
select 
  dp.*,
